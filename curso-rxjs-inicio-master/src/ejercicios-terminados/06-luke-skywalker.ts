...
(() =>{
    ...
    getRequest(`${SW_API}people/1`).pipe(
        ...
        //zip se usa para concatenar varias peticiones, combina ambos observables
        //pasando igualmente un stream en este caso se usa el of para que la respuesta sea stream
        switchMap(resp=>zip(of(resp),getRequest(resp.vehicles[0]))),
        map(
            ([personaje,vehiculo])=>({personaje,vehiculo})
        )
